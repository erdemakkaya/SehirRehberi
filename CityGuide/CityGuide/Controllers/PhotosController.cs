...
namespace CityGuide.Controllers
{
    ...
    {
        private IAppRepoistory _appRepository;
        private IMapper _mapper;
        private IOptions<CloudinarySettings> _cloudinaryConfig;
        private Cloudinary _cloudinary;

        public PhotosController(IAppRepoistory appRepoistory, IMapper mapper, IOptions<CloudinarySettings> cloudinarySettings)
        {
        ...
        public ActionResult AddPhotoForCity(int cityId, [FromForm]PhotoForCreationDto photoForCreationDto)
        {
            ...
            var curentUserId = 1;//int.Parse(User.FindFirst(ClaimTypes.NameIdentifier).Value);

            if (curentUserId!=city.UserId)
            {
                return Unauthorized();
            }
